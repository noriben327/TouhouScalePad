...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        trayMenu.Items.Add("TouhouScaleChangerを開く", null, (_, _) => Dispatcher.Invoke(ShowFromTray));
        ...
        {
            Text = "TouhouScaleChanger - ゲーム起動待機中",
    ...
    private void SaveProfileButton_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        {
            WpfMessageBox.Show(this, "ゲーム名と、有効なゲーム実行ファイルを指定してください。", "TouhouScaleChanger");
            return;
        }
        if (ProfileSizeComboBox.SelectedItem is not SizePreset sizePreset)
        {
            WpfMessageBox.Show(this, "拡大サイズを選択してください。", "TouhouScaleChanger");
            return;
        }
        ...
        {
            WpfMessageBox.Show(this, $"{processName}.exe は「{duplicate.GameName}」ですでに登録されています。", "TouhouScaleChanger");
            return;
        }
    ...
    {
        if (ProfileListBox.SelectedItem is not GameProfile profile) return;
        if (WpfMessageBox.Show(this, $"「{profile.GameName}」を削除しますか？", "TouhouScaleChanger",
    ...
    private void AddSizePresetButton_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        {
            WpfMessageBox.Show(this, "幅と高さは100～16384の整数で指定してください。", "TouhouScaleChanger");
            return;
        }
        if (_settings.SizePresets.Any(item => item.Width == width && item.Height == height))
        {
            WpfMessageBox.Show(this, "同じサイズのプリセットがすでにあります。", "TouhouScaleChanger");
            return;
        }
    ...
    private void DeleteSizePresetButton_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        if (preset.IsBuiltIn)
        {
            WpfMessageBox.Show(this, "標準プリセットは削除できません。", "TouhouScaleChanger");
            return;
        }
        if (_settings.GameProfiles.Any(item => item.SizePresetId == preset.Id))
        {
            WpfMessageBox.Show(this, "このサイズを使用しているゲームプロファイルがあります。先にプロファイルを変更してください。", "TouhouScaleChanger");
            return;
        }
    ...
    private void Runtime_OnStatusChanged(object? sender, RuntimeStatusChangedEventArgs e)
    {
        Dispatcher.InvokeAsync(() =>
        {
            ...
            _trayIcon.Text = e.ActiveGameCount > 0 ? "TouhouScaleChanger - ゲームへ適用中" : e.Monitoring ? "TouhouScaleChanger - ゲーム起動待機中" : "TouhouScaleChanger - 監視停止中";
